...
* [3. ERD](#3-erd)
* [4. 프로젝트를 진행하며 기억에 남는 것들](#4-프로젝트를-진행하며-기억에-남는-것들)
   * [4.1 HTTP 비동기의 활용](#41-http-비동기의-활용)
      * [4.1.1 문제 직면과 해결](#411-문제-직면과-해결)
      * [4.1.2 다른 곳으로의 확장](#412-다른-곳으로의-확장)
         * [4.1.2.1 값 유효 표시](#4121-값-유효-표시)
         * [4.1.2.2 자동 완성](#4122-자동-완성)
    * [4.2 첫 JavaScript](#42-첫-javascript)
       * [4.2.1 중복 코드 제거](#421-중복-코드-제거)
       * [4.2.2 업로드 이미지 관리](#422-업로드-이미지-관리)
* [5. 이 프로젝트를 진행하며 배운 것](#5-이-프로젝트를-진행하며-배운-것)
* [6. 계획했지만 여러 이유로 보류되거나 취소된 것들](#6-계획했지만-여러-이유로-보류되거나-취소된-것들)
* [7. 앞으로 할 것들](#7-앞으로-할-것들)
# 1. 개요
## 1.1 소개
...
>  댓글은 작성자만이 삭제할 수 있다. 또한 게시글의 추천을 할 수 있도록 추천 버튼 또한 도입했다. 추천한 게시글을 다시 추천하면 추천이 취소된다.
...
# 3. ERD
<br><img width=1000 height=700 src="https://github.com/tlsgkdns/ricu/assets/24753709/93bd6029-8279-4802-a360-73db9f2e3d12">
# 4. 프로젝트를 진행하며 기억에 남는 것들
이 프로젝트를 진행하며 여러 문제를 직면했다. 첫 웹 프로젝트인 만큼 여러 문제를 직면 및 해결했지만, 밑의 내용은 그 중 기억에 남는 것들 몇 개만 추려내었다.
## 4.1 HTTP 비동기의 활용
### 4.1.1 문제 직면과 해결
베이스가 된 프로젝트에서 이해가 힘들었던 부분 중 하나는 RestController를 활용해 댓글을 만들었단 점이였다. 나는 그것보단  @OneToMany를 사용하는 게 더 직관적이라 생각해서 그렇게 코딩했었다.

하지만 게시물 밑에 게시물 리스트를 추가하다가, 댓글과 게시물 리스트의 페이징이 중복되어 parameter를 어떻게 해야 할지에 대한 문제에 직면한다. 생각 끝에, 페이지를 갱신하지 않아도 되는 비동기를 떠올리고, 비동기 통신을 통해 댓글을 다시 코딩하였다. 이로써 페이징의 중복 문제를 해결하였고, 비동기에 대해 이해하는 좋은 기회가 되었다.
...
초기엔 버튼을 눌러서 값을 확인하는 방식을 채택했지만, 사용자로선 일일이 버튼을 눌러 사용할 수 있는 값을 확인하는 것보단 사용 가능 여부를 직관적으로 바로 알려주는 것이 더 편하리라 생각해 구현하였다.
...
하지만 일일이 이미지 업로드가 필요할 때마다 해당 코드를 복사하고 사용처에 맞게 수정하는 것은 관리가 쉽지 않다고 느껴졌다.<br>
갤러리 생성,수정,회원가입,유저정보 수정 등 총 네 군데에서 이것과 비슷한 코드를 사용하였고, 단순히 JavaScript에 익숙하지 않다는 이유로 중복 코드를 내버려 두기 힘들었다.<br>
그래서 JavaScript의 코드를 분석하고, 유추하는 과정을 거쳐 중복 코드를 획기적으로 줄일 수 있는 함수로 변환하는 데 성공하였다.
...
하지만, 코드를 구성하고 테스트를 진행하면서 공간을 쓸모없이 차지하는 것은 비효율적이다고 느꼈다.<br>
